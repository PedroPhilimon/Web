document.addEventListener('DOMContentLoaded', () => {
    ...
    // =========================================
    // 6. Redirección a la Página de Detalle (Mantenido)
    // =========================================
    document.querySelectorAll('.product-card .product-title a, .product-card > a').forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault(); 
            const card = link.closest('.product-card');
            const productName = card.querySelector('.product-title a').textContent.trim(); 
            window.location.href = `detail_product.html?product=${encodeURIComponent(productName)}`;
        });
    });
